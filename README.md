...
Client-side routing is when the client application is responsible for handling the routes (controls what happens when the url changes) while server-side routing is when the routing of the application is being handled by the server.
...
**C**: HTTP Methods: POST
**R**: HTTP Methods: GET
**U**: HTTP Methods: PUT, PATCH
**D**: HTTP Methods: DELETE
